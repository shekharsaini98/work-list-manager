...
export default function List({list, deleteItem, editItem }) {
    return(
        list.map((listtm)=>{
            const {id, title} = listtm;
            return (
                <article className="work-item" key={id}>
                    <p className="work-item-name">{title}</p>
                    <div className="work-item-action">
                    <button className="action-btn-edit" onClick={()=>editItem(listtm)}>
                    ...
                    <button className="action-btn-delete" onClick={()=>deleteItem(id)}>
